...
Thanks for using `cyanheads-mcp-server`. Bugs, feature requests, and documentation gaps all belong in an issue — that's where they get read and picked up.
...
`cyanheads-mcp-server` is built on [@cyanheads/mcp-ts-core](https://github.com/cyanheads/mcp-ts-core), which handles transports, auth, config, logging, and telemetry. Sorting out which layer broke saves everyone a round-trip:
...
- **[mcp-ts-core](https://github.com/cyanheads/mcp-ts-core/issues)** — a builder rejects valid input, `createApp()` fails on valid config, a `Context` method behaves contrary to its docs, or transport/auth misbehaves regardless of which tool you call.
...
Don't open a public issue for a vulnerability. Report it privately through GitHub's **Security** tab or email [security@caseyjhand.com](mailto:security@caseyjhand.com).
